...
sub sa_t_init {
  ...
  $scr = $ENV{'SCRIPT'};
  $scr ||= "$perl_cmd `which spamassassin`";
  ...
  $salearn ||= "$perl_cmd `which sa-learn`";
  ...
  (-f "t_Mail-SpamAssassin/test_dir") && chdir("t_Mail-SpamAssassin");        # run from ..
...
sub read_config {
  ...
  my $prefix = '';
  if (-f 't_Mail-SpamAssassin/test_dir') { $prefix = "t_Mail-SpamAssassin/"; }
